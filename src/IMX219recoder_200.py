...
timeLog             = []
#imgMem              = []    #イメージ格納用、Ver2.0.0以降はオミット
...
soundFile           = "/home/zero2/workspace/silent.mp3"
...
if GPIO.input(dip4Pin) == 0:
    exposure_time_absolute = 30
elif GPIO.input(dip4Pin) == 1:
    exposure_time_absolute = 100
print("exposure_time_absolute set to " + str(exposure_time_absolute))
...
def movieSave():
    global timeLog, codec, imgMem, stillNum, cutNum, capture, ledGPin, ledRPin, soundFile, frameNum
    GPIO.output(ledGPin, False)
    GPIO.output(ledRPin, True)
    ret, frame = capture.read()
    print(len(timeLog))
    print("len(imgMem) = " ,len(imgMem))
    ...
    fileName = filePath + "data/" + str(filmNum) + "_" + str(cutNum) + ".mp4"
...
#    os.system("ffmpeg -i videoonly -f lavfi -i aevalsrc=0 -shortest " + fileName)
#    os.system("ffmpeg -ar 48000 -t " + str(0.062*(len(imgMem) - 2)) + " -f s16le -acodec pcm_s16le -ac 2 -i /dev/zero -acodec libmp3lame -aq 4 /home/zero2/Workspace/silent.mp3")
#    addSound(fileName, soundFile)
